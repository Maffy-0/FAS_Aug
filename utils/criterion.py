...
class CrossEntropyLabelSmooth(torch.nn.Module):
    def __init__(self, num_classes=2, epsilon = 0, reduction='mean'):
        print("criterion's reduction ", reduction)
    ...
    def forward(self, input, target):  # pylint: disable=redefined-builtin
        ...
        if self.epsilon > 0.0:
            # epsilon = random.uniform(0, self.epsilon)
        ...
        if self.reduction in ['avg', 'mean']:
            ...
            return loss.sum()
        elif self.reduction == 'seperate': 
            loss_real = torch.tensor(0.).cuda()
            count_real = torch.tensor(0.).cuda() 
            loss_fake = 0
            count_fake = 0
            for i,_target in enumerate(target):
                if _target == 1:
                    ...
                    count_fake += 1
            if count_real != 0:
                loss_real = loss_real/count_real
            if count_fake != 0:
                loss_fake = loss_fake/count_fake
