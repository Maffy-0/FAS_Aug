...
# write seperately due to the diff transform
    def __getitem__(self, index):
        im = self.__read_image_from_zip__(index) # cv2 image, format [H, W, C], BGR
        im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        tensor = self.transform(im) # 1 im -> M im
        tensor = tensor.to(torch.float)
        
        try:
            changeLabel_list = self.transform.transforms[1].get_changeLabel()
            changeLabel = changeLabel_list[0]
            tar = self.face_label if not changeLabel else 0
            target = {'face_label':  tar}
        except Exception as e:
            target = {'face_label': self.face_label}
